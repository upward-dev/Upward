...
const schema = yup.object().shape({
  ...
  accomplishments: yup.string().required("Please describe what you accomplished"),
  experienceType: yup.string().required("Please select an option"),
  location: yup.string().required("Please enter a location"),
  from: yup.date().required("Start date is required").typeError("Date is required"),
...
export const Experience = ({isHidden,isSubmitted}:ExperienceProps) => {
  ...
  const onSubmit: SubmitHandler<InputValues> = ({title,accomplishments,experienceType,from,to}) => {
  ...
  function createExperience(data) {
    console.log(data);
    return experienceService.create(data)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={`${isHidden ? 'visually-hidden' : ''} d-md-flex flex-column w-100 justify-content-center align-items-center mt-5`}>
      <div className="col-md-7 col-lg-8">
        ...
        <label htmlFor="title" className="form-label">Enter your Experience Name</label>
      ...
      <div className="col-md-7 col-lg-8">
        <label htmlFor="accomplishments" className="form-label">Describe some of your accomplishments</label>
        <textarea id="accomplishments" {...register("accomplishments",{required:true})} className={`form-control ${errors.accomplishments?.message && "border border-danger"}`} aria-label="With textarea"></textarea>
      ...
      <div className="col-md-7 col-lg-8">
        ...
        <input id="location" {...register("location",{required:true})} className={`form-control ${errors.location?.message && "border border-danger"}`} aria-label="With textarea"></input>
        <p className="mt-1 text-danger">{errors.location?.message}</p>
      </div>
      <div className="col-md-7 col-lg-8">
        <label htmlFor="experienceType" className="form-label">Select a job/experience type</label>
        <select id="experienceType" {...register("experienceType")} className="form-select">

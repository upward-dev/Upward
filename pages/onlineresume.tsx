  ...
  return (
    <div className="d-flex flex-column min-vh-100 w-100 justify-content-center align-items-center">
      <section className="d-flex flex-column w-100 ml-2 mr-2 mt-2 justify-content-start align-items-center">
        <h2 className={`${(showProject || showExperience) && 'visually-hidden'} mt-5`}>Create Your Online Resume</h2>
        <div className="d-flex flex-row justify-content-around ">
          <button 
            onClick={(e) => setShowProject(!showProject)} 
            className={`${(showProject || showExperience) && 'visually-hidden'} btn btn-outline-secondary text-dark m-2 `}>Add a Project you've Completed
          </button>
          <button 
            onClick={(e) => setShowExperience(!showExperience)} 
            className={`${(showProject || showExperience) && 'visually-hidden'} btn btn-outline-secondary text-dark m-2 `}>Add a past Experience
          </button>
        </div>
